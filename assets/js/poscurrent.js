...
if (auth == undefined) {
    ...
    $(document).ready(function () {
        ...
        if (settings && settings.symbol) {
            $("#price_curr, #payment_curr, #change_curr", "#salesCustomerName_curr", "#salesCustomerPhone_curr").text(settings.symbol);
        }
        ...
        $.fn.submitter= async function (status, qcustomerQuery){
           
            // const customer = {"phoneNumber":"0772724747","referenceNumber":"ZRBT056","salesCurrency":"TZS","salesCustomer":"HAJI","salesItems":[
            //     {
            //     "discount":0,"itemId":0,"itemName":"SABUNI","price":2000,"quantity":2
            //     }
            //     ]
            //     };

                console.log("This is the customer query string" + JSON.stringify(event.value));
                    $.ajax({
                        ...
                        type: 'POST',
                        data: JSON.stringify(qcustomerQuery),
                        contentType: 'application/json; charset=utf-8',
                        cache: false,
                        processData: false,
                        ...
                        // ZRBResponse = {
                        //     salesCustomer: json.salesCustomer,
                        //     customerPhone: json.customerPhone,
                        //     salesCurrency: json.salesCurrency,
                        //     salesItems:json.salesItems,
                        //     receitpAmount:json.receiptAmount,
                        //     receiptNumber: json.receiptNumber,
                        //     taxExclussive: json.taxExclussive,
                        ...
                        //     responseNumber:json.responseNumber, 
                        //     getDailyTransaction: json.getDailyTransaction,
                        //     getAllTransaction:json.getAllTransaction, 
                        //     urn:json.urn, 
                        //     types:json.type, 
                        //     znumber:json.znumber,
                        // }
                        
                         await generateQR(json.receiptNumber.toString());
                         await $.fn.submitter(status, json);
        ...
        $.fn.addProductToCart = function (data) {
            ...
            item = {
                discount: 0,
                itemId: 0, //for taxable items ZRB requirement
                id: data._id,
        ...
        $.fn.submitDueOrder  = function (status) {
            ...
            cart.forEach(item => {
                ...
                items += "<tr><td>" + item.product_name + "</td><td>" + item.quantity + "</td><td>" + settings.symbol + " " + numberWithCommas(parseFloat(item.price).toFixed(2)) + "</td></tr>";
        ...
        $("#confirmPayment").on('click', function (customerQuery) {
            $("#receiptLoading").show();
            if ($('#payment').val() == "") {
                Swal.fire(
                    'Nope!',
                ...
                $(this).submitDueOrder(1,customerQuery);
            }
        });
...
function userFilter(users) {
    ...
    users.forEach(user => {
        ...
        $('#users').append(`<option value="${user}">salim</option>`);
    });
...
$.fn.viewTransaction = function (index) {
    ...
    if (allTransactions[index].paid != "") {
        payment = `<tr>
                ...
                <tr>
                    <td>Change</td>
                    <td>:</td>
                    <td>${settings.symbol + " "+ numberWithCommas(Math.abs(allTransactions[index].change).toFixed(2))}</td>
                </tr>
                <tr>
                    <td>Method</td>
                    <td>:</td>
    ...
    if (settings.charge_tax) {
        tax_row = `<tr>
                <td>Vat(${settings.percentage})% </td>
                <td>:</td>
                <td>${settings.symbol}${sellingTime ? parseFloat(allTransactions[index].tax).toFixed(2) : parseFloat(allTransactions[index].jsn.taxAmount).toFixed(2)}</td>
            </tr>`;
    }
    ...
    receipt = `<div style="font-size: 10px;">                            
        ...
        <span style="font-size: 22px;">${settings.store}</span> <br>
           <p>TAX PAYER: ${settings.address_one} <br>
